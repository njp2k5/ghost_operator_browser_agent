...
# Sentinel selector used to trigger lazy scan-based step generation in websocket.py
_SCAN_SENTINEL = "__funclink_scan__"


def _build_lazy_steps(target_url: str) -> list[dict]:
    """
    For non-Booking.com sites: create minimal placeholder steps.
    The WebSocket handler will scan the live page after navigation and
    replace the sentinel with LLM-generated steps based on actual visible fields.
    """
    return [
        {
            "step_number": 1,
        ...
        },
        {
            "step_number": 2,
            "action": "wait",
            "selector": _SCAN_SENTINEL,
            "instruction": "Scanning page to build your steps...",
            "prefill_value": None,
            "url": None,
            "is_skippable": False,
        },
...
async def generate_link(payload: GenerateLinkRequest, db: AsyncSession = Depends(get_db)):
    ...
    # 2. Generate step plan — hardcoded for Booking.com, lazy scan for everything else
    if _is_booking_com(payload.target_url):
        ...
        # Lazy: create navigate + sentinel. Real steps are generated by websocket.py
        # AFTER the browser has navigated and the actual page fields are scanned.
        # This is far more accurate than asking the LLM to guess the UI structure blind.
        steps_data = _build_lazy_steps(payload.target_url or "about:blank")
        from_memory = False
